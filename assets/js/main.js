...
// 		smartSpeed: 300,
	...
	$('.quantity-cart-plus').on('click',function() {
        ...
        var prices = parseFloat($('#price'+id+color).html().replace( /[^\d.]/g, '' ));
        var quan = parseInt($('#number'+id+color).html().replace( /[^\d.]/g, '' ));
        
        var main_price = parseFloat($('#main_price'+id+color).val());
        let rangenameone = $('#rangenameone'+id).val();
        console.log($('#main_price'+id+color).val());
        ...
        $.get(mainurl+'/cartupdate', function(response){
            ...
            $.each(response, function(i, cart){
                $.each(cart, function (index, data) {
                    ...
                    $("#goCart").append('<div class="cart-entry"> <div class="content"> <a class="title" href="'+url+'">'+data.title+'</a> <div class="quantity">'+language.quantity+': '+data.quantity+'</div><div class="price">'+currency+data.cost+'</div></div><div class="button-x"><i class="fa fa-close" onclick="getDelete('+data.product+', '+data.cartcolor+')"></i></div></div>');
        ...
        $.get(mainurl+'/cartdelete/'+id+'/'+color, function(response){
            ...
            $.each(response, function(i, cart){
                $.each(cart, function (index, data) {
                    ...
                    url = mainurl+'/product/'+data.product+'/'+title;
                    total = parseFloat(total) + parseFloat(data.cost);
                    $("#goCart").append('<div class="cart-entry"> <div class="content"> <a class="title" href="'+url+'">'+data.title+'</a> <div class="quantity">'+language.quantity+': '+data.quantity+'</div><div class="price">'+currency+data.cost+' FCFA</div></div><div class="button-x"><i class="fa fa-close" onclick="getDelete('+data.product+', '+data.cartcolor+')"></i></div></div>');
                    $('#grandttl').html(currency+total.toFixed(2));
                    $('#carttotal').html(currency+total.toFixed(2));
