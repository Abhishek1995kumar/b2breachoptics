...
use App\Http\Traits\PermissionSetController;
...
use DB;
use session;
use App\Services\SecurityPermissionInterface;
...
class LoginController extends Controller
{
    ...
    */
    ...
    public function login(SecurityPermissionInterface $permission, Request $request){
        ...
        if($userauth->status != 0){
            ...
            if(Auth::guard('web')->attempt($data)){
                ...
                $data = $permission->loginPermission($user);
                ...
                return redirect('admin')->withErrors(['password' => 'Invalid Password'])->onlyInput('password');
            }
        }
        else{
            return redirect('admin')->withErrors(['email' => 'You are not autherised to login'])->onlyInput('email');
        }
    }
