...
class PermissionManagerController extends Controller
{
    public function permissionCreateform(Request $request, $id)
    {
        ...
        $get_cat_or_permision_data = DB::table('product_list_cat_permission')->where('role_id','=',$id)->where('maincat', 'order')->get();
        $get_cat_pro_permision_data = DB::table('product_list_cat_permission')->where('role_id','=',$id)->where('maincat', 'product')->get();
        
        
        $role_wise_cate_per = array();
        foreach($get_cat_or_permision_data as $value){
            $role_wise_cate_per[] = $value->category_id;
        }
        
        $role_wise_cat_prod = array();
        foreach($get_cat_pro_permision_data as $value){
            $role_wise_cat_prod[] = $value->category_id;
        }
        ...
        return view('admin.permissioncreateform', compact('role', 'data', 'role_details', 'role_data_list', 'get_cat_permision_data', 'role_wise_cate_per', 'role_wise_cat_prod', 'categories'));
    }
    ...
    function Product_list_category_permission(Request $request)
    {
        $role_id = $_POST['role_id'];
        if($_POST['pcategory']) {
            $categorys = $_POST['pcategory'];
            DB::table('product_list_cat_permission')->where('role_id', $role_id)->where('maincat', 'product')->delete();
            foreach($categorys as $value){
                $insert_data = array('role_id' => $role_id,'category_id' => $value, 'maincat'=>'product');
                $count2 = DB::table('product_list_cat_permission')->where('role_id', '=', $role_id)->where(['category_id'=> $value, 'maincat'=>'product'])->count();
                if(!$count2){
                    DB::table('product_list_cat_permission')->insert($insert_data);
                }
            }
        }
        
        if($_POST['ordercategory']) {
            $ordcategorys = $_POST['ordercategory'];
            DB::table('product_list_cat_permission')->where('role_id', $role_id)->where('maincat', 'order')->delete();
            foreach($ordcategorys as $value){
                $insert_data = array('role_id' => $role_id,'category_id' => $value, 'maincat'=>'order');
                $count2 = DB::table('product_list_cat_permission')->where('role_id', '=', $role_id)->where(['category_id'=> $value, 'maincat'=>'order'])->count();
                if(!$count2){
                    DB::table('product_list_cat_permission')->insert($insert_data);
                }
            }
